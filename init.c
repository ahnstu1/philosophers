...
int	info_init(char **argv, t_info *info)
{
	...
	info -> ate = 0;
	info -> end = 0;
	info -> fs = malloc(sizeof(int) * info -> philo);
	memset(info -> fs, 1, sizeof(int) * info -> philo);
	if (info -> philo == -1 || info -> die == -1
		|| !info -> fs || info -> eat == -1 || info -> sleep == -1
...
int	philo_init(t_info *info, t_philo **philo)
{
	...
	*philo = malloc(sizeof(t_philo) * info -> philo);
	...
	while (idx < info -> philo)
	{
		...
		(*philo)[idx].left = 0;
		(*philo)[idx].right = 0;
		(*philo)[idx].left_state = &info -> fs[idx];
		(*philo)[idx].right_state = &(info -> fs[(idx + 1) % info -> philo]);
